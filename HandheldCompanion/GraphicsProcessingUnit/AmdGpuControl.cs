...
using static HandheldCompanion.ADLX.Adl2.NativeMethods;
...
namespace HandheldCompanion.GraphicsProcessingUnit
{
    ...
    public class AmdGpuControl
    {
        ...
        public ADLSingleSensorData[] GetiGpuSensors()
        {
            ...
            return adlpmLogDataOutput.Sensors;
        }
        ...
        public int? GetiGpuTemperature()
        {
            if (_adlContextHandle == nint.Zero || _iGPU == null) return null;
            if (ADL2_New_QueryPMLogData_Get(_adlContextHandle, ((ADLAdapterInfo)_iGPU).AdapterIndex, out ADLPMLogDataOutput adlpmLogDataOutput) != Adl2.ADL_SUCCESS) return null;

            ADLSingleSensorData gpuTemperature = adlpmLogDataOutput.Sensors[(int)ADLSensorType.PMLOG_TEMPERATURE_GFX];
            if (gpuTemperature.Supported == 0) return null;

            return gpuTemperature.Value;
        ...
        public int? GetGpuClock()
        {
            ...
            ADLSingleSensorData gpuUsage = adlpmLogDataOutput.Sensors[(int)ADLSensorType.PMLOG_CLK_GFXCLK];
            if (gpuUsage.Supported == 0) return null;
        ...
        public int GetFPSLimit()
        {
            ...
            Debug.WriteLine($"FPS Limit: {settings.ulACFPSCurrent}");
        ...
        public void KillGPUApps()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < appCount; i++)
                {
                    ...
                    {
                        Debug.WriteLine(appInfoArray[i].strFileName + ":" + appInfoArray[i].iGPUAffinity + "(" + appInfoArray[i].timeStamp + ")");
